...
public:
  ...
  DATA_INSTRUCTION(AsmFile &file, std::string &name, DX type,
      ...
      :
        ...
        DATA(file, *this), name(name), type(type), value(value),
        new_line(new_line) {}
  ...
  void handle_name(AsmFile &file) { file.write_to_file(name); }
...
DATA_INSTRUCTION donsus_data_instruction(AsmFile &file, std::string name,
                                         DX type, std::string value,
                                         std::string new_line) {
  // string view
  NASM_STRING string;
...
void donsus_codegen_section_vars_sym(utility::handle<donsus_symtable> symtable, AsmFile &file,
  ...
  auto inside_sym = [](auto &key, auto &value, AsmFile &file, DATA &data) {
    ...
    DATA_INSTRUCTION d_ins =
        donsus_data_instruction(file, value.get_name(), DB, key, "09AH");
    d_ins.fetch(file);
  };
